...
const getCurrentWeek = (): number => {
  // ISO 8601: weeks start on Monday and week 1 is the week with the first Thursday
  const now = new Date();
  const utcNow = new Date(
    Date.UTC(now.getFullYear(), now.getMonth(), now.getDate())
  );

  // Ensure the correct ISO year
  const day = utcNow.getUTCDay() || 7; // Sunday => 7
  utcNow.setUTCDate(utcNow.getUTCDate() + 4 - day);
  ...
  return (
    1 + Math.round((utcNow.getTime() - firstThursday.getTime()) / msInWeek)
  );
};
